...
- **「ブラウザによらずアクセス不可」の実現範囲**：VPN/DNSベースで最初から全ブラウザ・全アプリ共通でブロックする（案A）か、まず主要ブラウザ（Chromeなど）のみAccessibilityServiceで対応して後で拡張する（案B）か。メリデメ比較表を作成済み（`02 要件定義/MVPメリデメ比較_ブロック方式.md`）。ユーザーへの提示・確認待ち。
