...
[2026-06-25 追加要求]
- **追加要求①：時間帯指定での利用抑制**——ご飯中・寝る前など特定の時間帯のスマホ利用も抑制したい。指定の時間帯では在宅時より広いアプリ・サイト（LINE、メール等も含む）を制限対象にできるようにする。時間帯は複数設定可能にする。※在宅判定ルールとの関係（独立条件か、組み合わせるか）は未確認。
- **追加要求②：通知の時間帯コントロール**——時間帯によって通知を自動でマナーモード等に切り替えたい（例：仕事時間・寝る時間に自動切替）。既存のサイト/アプリブロックとは別軸の機能（通知制御）として扱う想定。
...
- **追加要求①の詳細**：時間帯ルールは在宅判定と独立して動くのか（在宅判定なしで時間帯だけでブロック発動するか）、それとも在宅判定と組み合わせる（在宅時かつ特定時間帯のみ、等）のか。時間帯ごとに制限対象アプリ・サイトのセットを別々に設定できる必要があるか。
- **追加要求②の詳細**：通知制御の対象は「マナーモード（サイレント/バイブ）」と「Do Not Disturb（Android標準のおやすみモード）」のどちらを想定しているか。時間帯設定は追加要求①の時間帯設定と共通化してよいか、別々に管理したいか。

## Current status (2026-06-25)

- 要件定義の初期段階。プラットフォーム・配布スコープ・サイトブロック実現方式（案C、対象Brave/Chrome/LINE）は確定。「解除の摩擦設計」が未回答のまま、ユーザーから追加要求2件（時間帯指定での利用抑制／通知の時間帯コントロール）が来ており、これらも合わせて確認待ち。
