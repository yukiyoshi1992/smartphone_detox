...
- **現在のアプリ構成（2026-06-25時点、画面・ルールモデルとも数回の見直し後の最新形）**：
  - **画面はTOP／設定／ルール管理（一覧）／ルール作成・編集の4つ**（`ui/`配下、Navigation Compose使用、`ui/AppNavHost.kt`でルーティング）。
    - TOP画面（`ui/TopScreen.kt`）：設定・ルール管理への遷移、ブロック機能全体ON/OFFスイッチ（`block/AppMasterSettings.kt`）、在宅状況・アクセシビリティ許可状況の表示。通知ルールはAlarmManagerで自動切替されるため、手動適用ボタン・関連の説明文は2026-06-25に削除済み（ユーザー指摘：「もういらないはず」）。
...
- **直後にユーザーから「致命的」な指摘**：通知ルールがTOP画面のボタン手動押下でしか評価されず、実用上意味がなかった。まず`androidx.work:work-runtime-ktx`で15分おきの`NotificationRuleWorker`を実装したが、「リアルタイムで反映してほしい、類似アプリは通知権限だけでそれができている」と再指摘。**最終的にAlarmManagerの厳密アラーム方式に変更**：`notification/NotificationAlarmScheduler.kt`が通知ルールの開始/終了時刻のうち直近のものを計算し`setExactAndAllowWhileIdle()`で1回限り予約、`notification/NotificationAlarmReceiver.kt`が発火時にリンガーモードを反映して次回分を再スケジュール、`notification/BootCompletedReceiver.kt`が再起動後に再スケジュール。厳密アラーム権限が無い場合は自動でinexactにフォールバックし、**追加の権限要求はユーザーに見せない**（既存の通知アクセス権限のみで動く設計）。WorkManagerの15分ポーリングはアラーム失敗時の保険として併用。**実機での自動切替の動作確認はこれから。**
- UATシナリオは「結果：未実施」が一部残るが、強制完遂は急がず実運用の中で随時更新する方針。**ルールの保存形式が2026-06-25中に2回変わったため、それより前に作成したルールは画面に表示されない（再作成が必要）。**
- **アクセシビリティ無効化検知・通知を追加**：`block/AccessibilityWatchdog.kt`がOS自動無効化リスク（旧UAT-A4、長らく緩和策未実装の既知issue）に対応。`BlockAccessibilityService.onUnbind()`で即座に検知、`NotificationRuleWorker`の15分おきチェックで保険。有効→無効の遷移時のみ通知（無効なままの間に毎回通知しない設計）。POST_NOTIFICATIONS権限の許可ボタンを設定画面に追加。**実機での動作確認はこれから。**
