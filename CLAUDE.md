...
- **実装順序②実装済み（実機検証待ち）**：`home`パッケージにWi-Fi在宅判定を実装（`HomeWifiSettings`でSSID設定の保存、`HomeWifiStatus`で現在のWi-Fi接続SSIDとの一致判定）。MainActivityに設定UI・権限リクエスト・状態確認ボタンを追加。commit&push済み、ユーザーのビルド・実機確認待ち。
