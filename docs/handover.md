...
1. **ユーザーに`git pull`して再ビルドしてもらい、新しいルール管理画面（一覧→編集）と通知ルールの扱いを確認してもらう。**ここから継続。
2. 確認OKなら、当初の予定どおり一通りモンキーテストしてもらい、洗い出された不具合・UI課題に個別対応する。**特に「ルールの時間帯条件が実際のブロック動作に反映される」という今回直した部分は重点的に確認してもらう**（UAT-D1が実質初回確認になる）。
...
8. **画面構成・データモデルのような「使い方が分かるか」という構造の指摘は、機能実装そのものより先に確認すべき観点**——機能を一通り実装してからモンキーテストする方針に変えた直後でも、画面構造（迷わず使えるか）・ルールの持ち方（アプリブロックとサイトブロックを同じ画面で一括管理すべきか等）はユーザーから見て最初に気になる点だった。今回は画面分割の直後にさらにルールモデル自体の再設計にもつながった。次に大きめの機能群を作るときは、個々の機能より先に「画面・ナビゲーション構造」と「データが何を単位に持つか」を一度提示して確認する方が手戻りが少ない。
9. **「Aの構成にしてほしい」という具体的な要望が来たら、その構成が暗黙的に意味する他の整合性（今回は通知ルールも同じ構造にすべきという話につながった）も併せて検討する**——ユーザーは1つの画面について要望したが、結果的に類似の別画面（通知）にも同じ要望が及んだ。次回も「これは他の似た画面にも展開すべきか」を一度確認・提案するとよい。
