...
1. ユーザーにローカルクローンで`git pull`してもらい、ビルド・実機確認を依頼する。**ここから継続。** ルールを追加し、「現在有効なルールを確認」ボタンで時間帯・曜日・在宅条件の組み合わせ通りに有効/無効が切り替わるか確認してもらう（日付をまたぐ時間帯も含めて）。
2. 動作確認できたら、④Brave/ChromeのURL検知→⑤通知マナーモード切替→⑥祝日API連携、の順に小さく動くものを積み上げていく（前回PJのパターンを踏襲）。実装するたびにUATシナリオ（`01 初回開発/03 UAT/UATシナリオ.md`）に該当ケースを追加・実施すること（カテゴリD関連のUAT-D1〜D3を更新）。
