...
- ユーザーから「アクセシビリティが許可→未許可になったときに通知を得ることはできますか？勝手に未許可になることがありますよね。気づけず」と要望。これは要件定義時点から`アーキテクチャ検討.md`に記載していた既知リスク（UAT-A4「アクセシビリティ権限がOSに自動でOFFにされた場合の検知」、長らく緩和策未実装だった項目）への対応依頼。
  - **対処**：`block/AccessibilityWatchdog.kt`を新設。前回チェック時の有効/無効状態をSharedPreferencesに記録し、有効→無効の遷移時のみ通知を出す設計（無効なままの間ずっと通知され続けるのを防ぐデバウンス）。
    - `BlockAccessibilityService.onUnbind()`をオーバーライドし、OSがサービスをアンバインドする瞬間（ユーザーが手動でOFFにした場合・OSが自動的に無効化した場合のどちらでも発生する）に即座に検知・通知。
    - 念のため`NotificationRuleWorker`（15分おき）でも同じチェックを実行し、何らかの理由でonUnbind検知が機能しなかった場合の保険とした。
    - Android 13+で通知の表示にはPOST_NOTIFICATIONS権限が必須なため、設定画面に許可ボタンを追加。
