...
1. （優先度低）対象デバイス（1台/複数台想定）を確認してもよいが、必須ではない。
2. **技術方針（アーキテクチャ）の検討に進む**：在宅判定（Wi-Fi/GPS）、時間帯×在宅判定のAND条件をどう組むか、AccessibilityServiceでのBrave/Chrome/LINEのUIノード調査、通知（マナーモード+バイブ）の自動切替実装方法、フォアグラウンドアプリ監視によるアプリブロックなど、前回PJの`02 要件定義/アーキテクチャ検討.md`的なドキュメントを`01 初回開発/02 設計・開発/`配下に作成し、ユーザーに提示・確認を取る。**次セッション最優先タスク。**
