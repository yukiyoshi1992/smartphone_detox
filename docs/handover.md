...
- Discordで提示し、4点についてユーザーから回答：
  1. サーバーなし構成 → **確定**。
  2. 在宅判定（Wi-Fi/GPS）→「メリデメ整理して」と要望あり。Wi-Fi/GPSの比較表をドキュメントに追加（バッテリー消費・屋内精度・検知できないケース・権限・実装コストの軸）。**Wi-FiのみでMVPを作る方針を提案、GPSは後で必要なら追加**——ユーザーの最終回答待ち。
  ...
  - ドキュメント・CLAUDE.mdに反映、commit&push予定。

### 次にやること（次セッション/次タスク最優先）
1. 在宅判定方式（Wi-FiのみのMVP提案）について、ユーザーの最終確認を取る。**ここから継続。**
