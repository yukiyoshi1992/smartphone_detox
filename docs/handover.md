...
- MVPメリデメ比較表を作成（`02 要件定義/MVPメリデメ比較_ブロック方式.md`）：案A（VPN/DNSベース全ブロック）と案B（主要ブラウザのみAccessibilityService）を実装コスト・保守性・抜け道の有無・開発速度・副作用/UX・アプリブロックへの貢献の軸で比較。要旨：アプリブロック自体は案A/B共通で別途フォアグラウンド監視が必要（比較に影響しない）。論点は「サイトブロックの抜け道のなさ」と「MVP到達速度」のトレードオフ——案Aは堅牢だが実装重い、案Bは最速だが対応ブラウザ以外は素通り。
...
2. 確定後、技術方針（アーキテクチャ、Accessibility対応ブラウザごとのUIノード調査含む）をまとめてユーザーに提示・確認を取る。
