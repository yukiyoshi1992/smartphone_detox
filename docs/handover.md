...
- ユーザーから「進めて」の承認を受け、**実装順序②（Wi-Fi在宅判定）を実装**：
  ...
  - ユーザーが再ビルド・「今接続中のWi-Fiを登録」→「在宅状態を確認」を実施 → **「在宅中」と正しく表示され成功**。UAT-B3をOKに更新。**実装順序②（Wi-Fi在宅判定）完了。**

### 次にやること（次セッション/次タスク最優先）
1. **実装順序③に着手**：ルールエンジン（時間帯×在宅条件のAND評価）。**ここから継続。**
2. その後④Brave/ChromeのURL検知→⑤通知マナーモード切替→⑥祝日API連携、の順に小さく動くものを積み上げていく（前回PJのパターンを踏襲）。実装するたびにUATシナリオ（`01 初回開発/03 UAT/UATシナリオ.md`）に該当ケースを追加・実施すること。
