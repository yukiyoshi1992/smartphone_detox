...
1. **ユーザーに`git pull`して再ビルドしてもらい、課題①〜⑫の修正を確認してもらう。**ルールはholidayMode導入で消えるため再作成してもらう。**ここから継続。**
2. **課題13（サイトブロックが効かない）は再現してlogcatを取得してもらい、`BlockAccessibility`タグのログから原因を特定する。**候補：AccessibilityServiceが実際に有効になっていない／Braveの`url_bar`リソースIDが想定と異なる／ホーム判定がfalseになっている、のいずれか。ログがあれば一発で切り分けられるはず。
3. 上記が落ち着いたら、当初の予定どおり一通りモンキーテストを再開し、UATシナリオ（`01 初回開発/03 UAT/UATシナリオ.md`）の「結果：未実施」を更新していく。
